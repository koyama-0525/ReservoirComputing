...
/*====================  Loop: Reservoir realizatons (Topology) ====================*/
   ...
   for(no=1; no<=n_rsv; no++){ // no-Loop (reservoir ralization)
...
/*====================  Loop: p,sigma,alpha  ====================*/
     ...
     for(m=0; m<=m_max; m++){  // p-Loop (mixture rate)
...
/*====================  Training phase  ====================*/
           ...
           for(cls=0; cls<=n_cls-1; cls++){  // 学習する識別器の番号

             count_train=0;

             for(c1=0; c1<=n_cls-1; c1++){  // train. data class
...
/*====================  Validation phase  ====================*/
           ...
           for(cls=0; cls<=n_cls-1; cls++){  // 誤差測定する識別器の番号
             for(c1=0; c1<=n_cls-1; c1++){  // val. data class
...
{ 
   ...
   sum=0.0;
   for(i=tau[cls]+1; i>=1; i--){ //for(i=tau; i>=1; i--){
     yt[i]=yt[i-1];
     sum=sum+yt[i];
   }
...
*/
   //... cutt-off bound ...
   if(yt[0]>1.0){ yt[0]=1.0; flag_cut=1;}
   else if(yt[0]<-1.0){yt[0]=-1.0; flag_cut=1;}
